...
def process_demo(args, config):
    ...
    while True:
        ...
        if variables['crawler_cache_dir_index'] < variables['cache_dir_index']: # nothing to update
            print('Nothing to update!')
            sleep(args.sleep_time)
        elif variables['crawler_cache_dir_index'] > variables['cache_dir_index']: # invalid update step
            raise Exception('[!] The value of crawler_cache_dir_index cannot be higher than cache_dir_index.')
            ...
            if demo_from_scratch_flag:
                ...
                cmd = 'python main.py --mode build_indices_local --pdf_ip {0} --pdf_port {1} \
                    --es_ip {2} --es_port {3} --delete_indices 1 --local_mongo_drop_flag 1\
                    '.format(args.pdf_ip, args.pdf_port, args.es_ip, args.es_port)
                os.system(cmd)
                print('cmd: ', cmd)
            else:
                cmd = 'python main.py --mode build_indices_local --pdf_ip {0} --pdf_port {1} \
                    --es_ip {2} --es_port {3} --delete_indices 0 --local_mongo_drop_flag 0\
                    '.format(args.pdf_ip, args.pdf_port, args.es_ip, args.es_port)
                os.system(cmd)
                print('cmd: ', cmd)
            ...
            os.system(cmd)
            print('cmd: ', cmd)
            print('Successfully update dir: [{0}] !'.format(variables['crawler_cache_dir_index']))
...
if __name__ == '__main__':
    """Usage:
            demo:
                ...
                2. python run.py --mode demo --pdf_ip PDF_IP --pdf_port PDF_PORT --es_ip ES_IP \
    ...
    parser.add_argument("--sleep_time", type=int, default='32', help='update interval')
    parser.add_argument("--config", type=str, default='./config.json', help='path to config file')
    parser.add_argument("--var_file", type=str, default='./varFile.json', help='path to varFile.json')
